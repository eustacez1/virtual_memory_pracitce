...
int swap_in( int page_number,int frame_number){
    ...
    FILE* backing_store_file = fopen(backing_store, "r+");
...
int mem_handler(char* filepath) {
    ...
    create_queue(page_replace_queue,PAGE_TABLE_SIZE);
    tlb_init(tlb);
    ...
    while (fgets(line, sizeof(line), addr_input_file)) {
        ...
        if (frame_number == -1) {
            ...
            // If frame_number is -1, then it was not found in the backing store
            switch (frame_number) {
                ...
                default:
                    printf("Error: Frame number not found in disk.\n");
                    exit(EXIT_FAILURE);
